...
struct IconSet<Content: View>: Encodable {
    ...
    init(idioms: Set<Idiom>, view: Content, name: String) {
        images = [
            .init(idiom: .iPad, size: CGSize(width: 20, height: 20), scale: .oneX, placeholder: !idioms.contains(.iPad), name: name),
            .init(idiom: .iPad, size: CGSize(width: 20, height: 20), scale: .twoX, placeholder: !idioms.contains(.iPad), name: name),

            .init(idiom: .iPad, size: CGSize(width: 29, height: 29), scale: .oneX, placeholder: !idioms.contains(.iPad), name: name),
            .init(idiom: .iPad, size: CGSize(width: 29, height: 29), scale: .twoX, placeholder: !idioms.contains(.iPad), name: name),

            .init(idiom: .iPad, size: CGSize(width: 40, height: 40), scale: .oneX, placeholder: !idioms.contains(.iPad), name: name),
            .init(idiom: .iPad, size: CGSize(width: 40, height: 40), scale: .twoX, placeholder: !idioms.contains(.iPad), name: name),

            .init(idiom: .iPad, size: CGSize(width: 76, height: 76), scale: .oneX, placeholder: !idioms.contains(.iPad), name: name),
            .init(idiom: .iPad, size: CGSize(width: 76, height: 76), scale: .twoX, placeholder: !idioms.contains(.iPad), name: name),

            .init(idiom: .iPad, size: CGSize(width: 83.5, height: 83.5), scale: .twoX, placeholder: !idioms.contains(.iPad), name: name),


            .init(idiom: .iPhone, size: CGSize(width: 20, height: 20), scale: .twoX, placeholder: !idioms.contains(.iPhone), name: name),
            .init(idiom: .iPhone, size: CGSize(width: 20, height: 20), scale: .threeX, placeholder: !idioms.contains(.iPhone), name: name),

            .init(idiom: .iPhone, size: CGSize(width: 29, height: 29), scale: .twoX, placeholder: !idioms.contains(.iPhone), name: name),
            .init(idiom: .iPhone, size: CGSize(width: 29, height: 29), scale: .threeX, placeholder: !idioms.contains(.iPhone), name: name),

            .init(idiom: .iPhone, size: CGSize(width: 40, height: 40), scale: .twoX, placeholder: !idioms.contains(.iPhone), name: name),
            .init(idiom: .iPhone, size: CGSize(width: 40, height: 40), scale: .threeX, placeholder: !idioms.contains(.iPhone), name: name),

            .init(idiom: .iPhone, size: CGSize(width: 60, height: 60), scale: .twoX, placeholder: !idioms.contains(.iPhone), name: name),
            .init(idiom: .iPhone, size: CGSize(width: 60, height: 60), scale: .threeX, placeholder: !idioms.contains(.iPhone), name: name),

            .init(idiom: .marketing, size: CGSize(width: 1024, height: 1024), scale: .oneX, placeholder: !idioms.contains(.marketing), name: name)
        ]
...
//        try? FileManager.default.removeItem(at: iconSetUrl)
        ...
        //create the main icon which name will go to the info.plist CFBundleIcons~ipad
        let mainIconName = images[8].name + ".png"
        let data = try content.generateImageData(size: images[8].size * images[8].scale.multiplier)
        try data.write(to: iconSetUrl.appendingPathComponent(mainIconName))
